...
if [[ $PRE_TRANSFER_COMMANDS == "" ]]; then
	echo "Nothing to do, no pre transfer commands configured."
	exit 0
fi
...
if [[ $TRANSFER_METHOD == "ssh"* ]]; then
	for H in $TARGET_HOSTS; do
		...
		run "ssh -p ${TARGET_PORT} -T ${TARGET_USER}@${H} \"cd ${TARGET_PATH} && ${PRE_TRANSFER_COMMANDS}\""
		msg "<<<<<<<<<<<<<<<<<<<<<<<<<<<<< ${TARGET_USER}@${H}:${TARGET_PATH} "
	done
else
